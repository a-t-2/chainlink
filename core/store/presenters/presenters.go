...
func NewConfigPrinter(config config.GeneralConfig) (ConfigPrinter, error) {
	...
	ocr2KeyBundleID, _ := config.OCR2KeyBundleID()
	ocr2TransmitterAddress := ""
	ocr2TransmitterAddressRaw, err := config.OCR2TransmitterAddress()
	if err != nil {
		ocr2TransmitterAddress = ocr2TransmitterAddressRaw.String()
	}
	ocr2P2PBootstrapPeers := ""
	ocr2P2PBootstrapPeersRaw, _ := config.OCR2P2PBootstrapPeers()
	if err != nil {
		ocr2P2PBootstrapPeers = strings.Join(ocr2P2PBootstrapPeersRaw, ",")
	}
	ocr2P2PPeerID := ""
	ocr2P2PPeerIDRaw, err := config.OCR2P2PPeerID()
	if err != nil {
		ocr2P2PPeerID = ocr2P2PPeerIDRaw.String()
	}
	ocr2P2PV2Bootstrappers := ""
	for _, peer := range config.OCR2P2PV2Bootstrappers() {
		ocr2P2PV2Bootstrappers += fmt.Sprintf("%s@[%s] ", peer.PeerID, strings.Join(peer.Addrs, ", "))
	}

	ocr2P2PV2AnnounceAddresses := strings.Join(config.OCR2P2PV2AnnounceAddresses(), ", ")
	ocr2P2PV2ListenAddresses := strings.Join(config.OCR2P2PV2ListenAddresses(), ", ")
	...
	return ConfigPrinter{
		EnvPrinter: EnvPrinter{
			...
			OCR2BlockchainTimeout:                  config.OCR2BlockchainTimeout(),
			OCR2ContractPollInterval:               config.OCR2ContractPollInterval(),
			OCR2ContractSubscribeInterval:          config.OCR2ContractSubscribeInterval(),
			...
			OCR2P2PPeerID:                           ocr2P2PPeerID,
			OCR2P2PPeerstoreWriteInterval:           config.OCR2P2PPeerstoreWriteInterval(),
			OCR2P2PV2AnnounceAddresses:              ocr2P2PV2AnnounceAddresses,
			OCR2P2PV2Bootstrappers:                  ocr2P2PV2Bootstrappers,
			OCR2P2PV2DeltaDial:                      config.OCR2P2PV2DeltaDial(),
			OCR2P2PV2DeltaReconcile:                 config.OCR2P2PV2DeltaReconcile(),
			OCR2P2PV2ListenAddresses:                ocr2P2PV2ListenAddresses,
		},
	}, nil
}
