...
func setupNodeOCR2(t *testing.T, owner *bind.TransactOpts, port uint16, dbName string, b *backends.SimulatedBackend) (*cltest.TestApplication, string, common.Address, ocr2key.EncryptedKeyBundle, *configtest.TestEVMConfig, func()) {
	config, _, ormCleanup := heavyweight.FullTestORM(t, fmt.Sprintf("%s%d", dbName, port), true)
	config.GeneralConfig.Overrides.FeatureOffchainReporting2 = null.BoolFrom(true)

	app, appCleanup := cltest.NewApplicationWithConfigAndKeyOnSimulatedBlockchain(t, config, b)
	_, _, err := app.GetKeyStore().OCR2().GenerateEncryptedP2PKey()
	require.NoError(t, err)

	p2pIDs := app.GetKeyStore().OCR2().DecryptedP2PKeys()
	require.Len(t, p2pIDs, 1)
	peerID := p2pIDs[0].MustGetPeerID()

	config.GeneralConfig.Overrides.OCR2P2PPeerID = peerID
	config.GeneralConfig.Overrides.OCR2P2PListenPort = port
	p2paddresses := []string{
		fmt.Sprintf("127.0.0.1:%d", port),
	}
	config.GeneralConfig.Overrides.OCR2P2PV2ListenAddresses = p2paddresses
	config.GeneralConfig.Overrides.OCR2P2PV2AnnounceAddresses = p2paddresses
	...
	_, kb, err := app.GetKeyStore().OCR2().GenerateEncryptedOCR2KeyBundle()
	require.NoError(t, err)
...
func TestIntegration_OCR2(t *testing.T) {
	...
	appBootstrap, bootstrapPeerID, _, _, cfg, cleanup := setupNodeOCR2(t, owner, bootstrapNodePort, "bootstrap", b)
	defer cleanup()

	var (
		oracles      []confighelper2.OracleIdentityExtra
		transmitters []common.Address
		kbs          []ocr2key.EncryptedKeyBundle
	...
	for i := uint16(0); i < 4; i++ {
		...
		cfg.GeneralConfig.Overrides.OCR2ObservationGracePeriod = 100 * time.Millisecond

		// Supply the bootstrap IP and port as a V2 peer address
		cfg.GeneralConfig.Overrides.OCR2P2PV2Bootstrappers = []commontypes.BootstrapperLocator{
		...
		oracles = append(oracles, confighelper2.OracleIdentityExtra{
			OracleIdentity: confighelper2.OracleIdentity{
				OnChainSigningAddress: dkb.OnchainKeyring.SigningAddress().Bytes(),
				TransmitAccount:       ocrtypes2.Account(transmitter.String()),
				OffchainPublicKey:     dkb.OffchainKeyring.OffchainPublicKey(),
				PeerID:                peerID,
			},
			ConfigEncryptionPublicKey: dkb.OffchainKeyring.ConfigEncryptionPublicKey(),
		})
	}
	...
	ocrJob, err := offchainreporting2.ValidatedOracleSpecToml(cfg, fmt.Sprintf(`
...
`, ocrContractAddress))
	...
	for i := 0; i < 4; i++ {
		...
		ocrJob, err := offchainreporting2.ValidatedOracleSpecToml(apps[i].GetEVMConfig(), fmt.Sprintf(`
type               = "offchainreporting2"
schemaVersion      = 1
